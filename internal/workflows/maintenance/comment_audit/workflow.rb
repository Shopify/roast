...
# Guards against comment drift by treating a single contiguous `#` prose block as the atomic unit of
# audit: both a definition's doc comment and an inline comment that documents no definition. It enumerates the
# in-scope comment blocks (see Targets below), asks an agent to verify each one in isolation against
# the real implementation, groups the stale ones into a single PR, and opens it only when the `apply`
# flag is passed. Without `apply` it is read-only: it prints the findings and opens nothing.
#
# Parameters:
...
#   bin/roast execute internal/workflows/maintenance/comment_audit/workflow.rb
#
#   # dry-run only a directory or a single file (shell globs are expanded too):
#   bin/roast execute internal/workflows/maintenance/comment_audit/workflow.rb lib/roast/cog
#   bin/roast execute internal/workflows/maintenance/comment_audit/workflow.rb lib/roast/cog/output.rb
#
#   # open the PR of fixes on top of the current branch:
#   bin/roast execute internal/workflows/maintenance/comment_audit/workflow.rb -- apply
#
#   # scope the audit and apply in one run:
#   bin/roast execute internal/workflows/maintenance/comment_audit/workflow.rb lib/roast/cog -- apply
...
execute do
  ...
  # Phase 1: enumerate atomic comment units using prism: a doc-comment block above each definition,
  # plus any inline comment block that lives inside a body.
  ruby(:enumerate) do
    require "prism"
    ...
    # Comment patterns that are never human prose and must not be counted as part of a comment block.
    ...
    # Accumulates one audit unit per comment block (a definition's doc comment or an inline comment),
    ...
    scope_files.each do |file|
      ...
      # Record every definition's anchor line (its `def`/`class`/`module`/constant/attr line) with its
      # fully-qualified symbol and kind. The walk calls `emit` for each definition it finds; the prose
      # scan below reads this map to tell a doc comment (a block sitting directly above an anchor) from
      # an inline comment.
      anchors = {}
      emit = ->(anchor, symbol, kind) { anchors[anchor] = { symbol: symbol, kind: kind } }

      # Recursively walk the AST to record each definition's anchor via `emit`. Each branch handles a
      # form a doc comment can sit above; method bodies are not descended into, so comments inside them
      # are treated as inline by the scan below.
      walk = nil
      walk = lambda do |node, ns, singleton|
        ...
        when Prism::StatementsNode
          ...
          # When Prism sees a module/class body with a module-level `rescue` wrapped in a begin; recurse into it.
      ...
      # Start the walk at the file root; the recursion fills in `anchors`.
      walk.call(result.value, "", false)

      # A prose line is a real comment line carrying human text: not a magic/directive, RBS signature
      # (`#:`/`#|`), or rubocop line. Bare `#` lines join a block (they are trimmed off each end below).
      prose = ->(ln) { doc.call(ln) && !hard.call(ln) && !body.call(ln).match?(sig) && !body.call(ln).match?(rubocop) }

      # Scan the file into maximal contiguous prose blocks; each block is one audit unit. It is a
      # definition's doc comment when the code directly below it is a recorded anchor (skipping the
      # sig/rubocop/blank zone that may sit between a comment and its def); otherwise it is inline.
      ln = 1
      while ln <= lines.length
        unless prose.call(ln)
          ln += 1
          next
        end
        run_start = ln
        ln += 1 while ln <= lines.length && prose.call(ln)
        run_end = ln - 1
        run_start += 1 while run_start <= run_end && blank.call(run_start)
        run_end -= 1 while run_end >= run_start && blank.call(run_end)
        next if run_start > run_end

        # Look at the code line below the block; `anchor` is set when it begins a definition.
        below = run_end + 1
        below += 1 while below <= lines.length && doc.call(below) &&
            (body.call(below).match?(sig) || body.call(below).match?(rubocop) || blank.call(below))
        anchor = below <= lines.length ? anchors[below] : nil

        if anchor
          # Doc comment: pull the definition's RBS signature lines (if any) from the skipped zone.
          sig_lines = ((run_end + 1)...below).select { |n| body.call(n).match?(sig) }.map { |n| body.call(n) }
          units << {
            file: file,
          ...
          # Inline comment: a prose block that documents no definition.
          units << {
            file: file,
            symbol: run_start == run_end ? "inline @ L#{run_start}" : "inline @ L#{run_start}-#{run_end}",
            kind: "inline",
            start_line: run_start,
            end_line: run_end,
            signature: nil,
  ...
  ruby(:apply) do
    ...
    # Builds the PR body: one section per file, one bullet per stale comment (doc or inline), with its confidence and issues.
    body = +"Automated comment audit on top of `#{original}` (base `#{base_short}`).\n\n"
    files.each do |file|
      body << "### `#{file}`\n\n"
    ...
    pr_plan =
      if failing.empty?
        ...
        [{
          id: base_short,
          base: original,
          branch: "comment-audit/#{original.tr("/", "-")}",
          title: "Fix #{failing.size} stale comment(s) across #{files.size} file(s)",
          body: body,
          files: files,
          verdicts: failing,
        }]
  ...
  ruby(:summary) do
    ...
    if arg?(:apply)
      ...
      if url
        ...
        puts "no stale comments; nothing to open."
      end
    else
...
# Named scope: verifies one atomic comment unit (a doc comment or an inline comment)
