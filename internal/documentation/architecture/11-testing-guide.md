...
│   └── functional/
│       └── roast_examples_test.rb      # 30 workflow integration tests
└── fixtures/
...
All shared test infrastructure lives in `test/test_helper.rb` (251 lines). Everything is globally available to all test files.
...
**Source**: `test/test_helper.rb` lines 174–218
...
**How it works**: The EventMonitor logs stdout lines with a `❯` (or `❙`) marker and stderr lines with `❯❯` (or `❙❙`). This method:
1. Scans lines for the log prefix pattern (`/^[DIWEFA], \[/`)
2. Identifies `❯❯`/`❙❙` lines as stderr, `❯`/`❙` lines as stdout
3. Also uses a `↓` header-match pattern for initial stream identification
4. Continuation lines (without log prefix) belong to the current stream
5. Returns `[stdout_string, stderr_string]`
...
**Source**: `test/test_helper.rb` lines 136–163
...
**Source**: `test/examples/support/functional_test.rb` (61 lines)
...
- `TestInput` and `TestOutput` are defined outside the `TestCog` class body, using the `TestCogSupport` module namespace (required by `Roast/NoTestClassNesting` cop)
...
**Source**: `test/test_helper.rb` lines 232–251
