...
module Roast
  module Workflow
    class ContextManagerTest < ActiveSupport::TestCase
      def setup
        @config = Roast::Workflow::ContextManagementConfig.new(
          enabled: true,
          strategy: "truncation",
          threshold: 0.75,
          max_tokens: nil,
      ...
      test "count_tokens with character ratio for non-OpenAI models" do
        @context_manager = ContextManager.new(config: @config, model: "unknown-model")
        text = "Hello world"
      ...
      test "uses model-specific character-to-token ratio for Claude models" do
        claude_config = Roast::Workflow::ContextManagementConfig.new(
          enabled: true,
          strategy: "truncation",
          threshold: 0.75,
      ...
      test "falls back to config ratio when model has no specific ratio" do
        unknown_config = Roast::Workflow::ContextManagementConfig.new(
          enabled: true,
          strategy: "truncation",
          threshold: 0.75,
      ...
      test "falls back to default ratio when both model and config have no ratio" do
        minimal_config = Roast::Workflow::ContextManagementConfig.new(
          enabled: true,
          strategy: "truncation",
          threshold: 0.75,
        )
      ...
      test "llm_summarization strategy creates summary message" do
        ...
        manager = ContextManager.new(config: @config, model: @model, workflow: mock_workflow)
        large_transcript = [
          { "role" => "system", "content" => "System prompt" },
          { "role" => "user", "content" => "x" * 30000 }, # Large content to trigger compaction
      ...
      test "summarization_model selects appropriate models" do
        config = Roast::Workflow::ContextManagementConfig.new(enabled: true, strategy: "llm_summarization")

        # Test key model mappings
        manager = ContextManager.new(config: config, model: "gpt-4")

        manager.instance_variable_set(:@model, "gpt-4o")
        assert_equal "gpt-4o-mini", manager.send(:summarization_model)

        manager.instance_variable_set(:@model, "claude-3.5-sonnet-20241022")
        assert_equal "claude-3-5-haiku-20241022", manager.send(:summarization_model)

        manager.instance_variable_set(:@model, "gemini-2.0-pro")
        assert_equal "gemini-2.0-flash", manager.send(:summarization_model)
      end

      test "summarization_model respects override" do
        config = Roast::Workflow::ContextManagementConfig.new(
          enabled: true,
        ...
        assert_equal "custom-model", manager.send(:summarization_model)
      end

      test "summarization_model handles experimental models" do
        config = Roast::Workflow::ContextManagementConfig.new(enabled: true, strategy: "llm_summarization")
        manager = ContextManager.new(config: config, model: "gpt-4")
        ...
        assert_equal "unknown-model", manager.send(:summarization_model)
      end
    end
  end
end
