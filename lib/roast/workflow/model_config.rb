...
module Roast
  module Workflow
    # ModelConfig provides token limits and character-to-token ratios for various LLM models
    # Used by ContextManager to determine when context compaction is needed
    class ModelConfig
      ...
      MODEL_CONFIG = {
        ...
        "claude-3-opus" => { max_tokens: 200_000, character_to_token_ratio: 0.33 },
      ...
      class << self
        ...
        #
        # @param model [String] The model name (e.g., "gpt-4-turbo-2024")
        # @return [Integer] Maximum token limit for the model
        def max_tokens_for(model)
          return MODEL_LIMITS["default"] if model.nil? || model.empty?
          ...
          matching_key = MODEL_LIMITS.keys.sort_by(&:length).reverse.find { |key| model.start_with?(key) }
        ...
        def character_to_token_ratio_for(model)
          return MODEL_CONFIG["default"][:character_to_token_ratio] if model.nil? || model.empty?
          return MODEL_CONFIG[model][:character_to_token_ratio] if MODEL_CONFIG.key?(model)

          # Try to find a matching prefix
          matching_key = MODEL_CONFIG.keys.sort_by(&:length).reverse.find { |key| model.start_with?(key) }
          return MODEL_CONFIG[matching_key][:character_to_token_ratio] if matching_key

          # For completely unknown models, return default
          MODEL_CONFIG["default"][:character_to_token_ratio]
        end

        private

        def get_model_config(model)
          return MODEL_CONFIG["default"] if model.nil? || model.empty?
