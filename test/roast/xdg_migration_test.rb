...
module Roast
  class XDGMigrationTest < ActiveSupport::TestCase
      ...
      with_fake_xdg_env do |temp_dir|
        output = capture_io do
          ...
          Dir.chdir(temp_dir) do
            Roast::XDGMigration.migrate(nil, true)
          end
        end
    ...
    test "migrate migrates all types together" do
      ...
      with_fake_xdg_env do |temp_dir|
        ...
        output = capture_io do
          ...
          Roast::Helpers::Logger.reset
          Dir.chdir(temp_dir) do
            Roast::XDGMigration.migrate(temp_dir, true)
          end
        end
    ...
    test "migrate_directory prompts for overwrite when file exists and user confirms" do
      with_fake_xdg_env do |temp_dir|
        ...
        # Mock the prompt to confirm overwrite
        ::CLI::UI::Prompt.expects(:confirm).with("File already exists at #{File.join(xdg_cache_dir, "existing.cache")}. Do you want to overwrite it?").returns(true)
    ...
    test "migrate_directory skips overwrite when file exists and user declines" do
      with_fake_xdg_env do |temp_dir|
        # Create legacy cache with files that will conflict
        legacy_cache_dir = create_legacy_directory(temp_dir, "cache")
        create_test_file(legacy_cache_dir, "existing.cache", "legacy content")
        ...
        # Mock the prompt to decline overwrite
        ::CLI::UI::Prompt.expects(:confirm).with("File already exists at #{File.join(xdg_cache_dir, "existing.cache")}. Do you want to overwrite it?").returns(false)
    ...
    test "migrate_file prompts for overwrite when file exists and user confirms" do
      with_fake_xdg_env do |temp_dir|
        ...
        capture_io do
          Roast::Helpers::Logger.reset
          Dir.chdir(temp_dir) do
            Roast::XDGMigration.migrate(nil, false)
          end
        end
    ...
    test "migrate_directory handles multiple files with mixed overwrite responses" do
      with_fake_xdg_env do |temp_dir|
        ...
        # Mock the prompts - confirm first, decline second
        ::CLI::UI::Prompt.expects(:confirm).with("File already exists at #{File.join(xdg_cache_dir, "file1.cache")}. Do you want to overwrite it?").returns(true)
        ::CLI::UI::Prompt.expects(:confirm).with("File already exists at #{File.join(xdg_cache_dir, "file2.cache")}. Do you want to overwrite it?").returns(false)
        ...
        # file1 should be overwritten, file2 should be preserved, file3 should be migrated
        assert_file_content(File.join(xdg_cache_dir, "file1.cache"), "legacy file1")
        assert_file_content(File.join(xdg_cache_dir, "file2.cache"), "existing file2")
